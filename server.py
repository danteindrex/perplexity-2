...
class PerplexitySearchTool(BaseTool):
    ...
    def run(self, query: str) -> str:
        """Execute search synchronously by running async in event loop"""
        return asyncio.run(self._arun(query))

# CrewAI Agents Definition
def create_analysis_agent(mcp_client) -> Agent:
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("job_research_system:mcp.app", host="0.0.0.0", port=8000, reload=True)